...
// Lays every tip out at the panel's own width and reports each height, so the body
// can be sized to the tallest of them rather than to a hand-tuned constant that has
// to be re-guessed whenever a tip is edited.
//
...
//
// `aria-hidden` keeps a screen reader from reading all five tips on top of the one
// actually on show.
export function TipSizer({ onMeasure }: { onMeasure: (height: number) => void }) {
  return (
